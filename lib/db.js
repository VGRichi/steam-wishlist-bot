...
async function upsertUserRegion(chatId, regionCode) {
  const { error } = await supabase
    .from('users')
    .upsert({ chat_id: chatId, region_code: regionCode.toUpperCase(), pending_action: null });
  if (error) throw error;
}

// pending_action is a tiny piece of conversation state (e.g. "awaiting_region")
// so the webhook knows how to interpret the next plain-text message from a
// user, since Vercel functions have no memory between requests.
async function setPendingAction(chatId, action) {
  const { error } = await supabase
    .from('users')
    .upsert({ chat_id: chatId, pending_action: action });
  if (error) throw error;
}
...
async function addTrackedGame(chatId, appId, gameName) {
  ...
  const { error } = await supabase
    .from('tracked_games')
    .insert({ chat_id: chatId, app_id: appId, game_name: gameName });
  if (error) throw error;
  return { ok: true };
}
...
async function clearTrackedGames(chatId) {
  const { error } = await supabase
    .from('tracked_games')
    .delete()
    .eq('chat_id', chatId);
