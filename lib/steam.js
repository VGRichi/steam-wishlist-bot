...
// Searches Steam's storefront search (same relevance ranking as the site's
// own search box). Unofficial endpoint - if it ever breaks, callers should
// fall back to searchGamesFallback().
async function searchGames(query, region = 'US') {
  ...
  return (data.items || [])
    .filter((item) => item.type === 'game') // exclude DLC/soundtracks/etc.
    .slice(0, 5)
