...
// Common Steam-supported region codes. Not exhaustive, but covers the vast
// majority of real users. If someone's region isn't here, appdetails will
// still usually work with it - this is just a friendly upfront sanity check.
...
// Extracts an App ID from a raw number or a Steam store URL. Returns null if
...
async function searchGames(query, region = 'US') {
  ...
  // storesearch returns items with type "app" for regular games/DLC/software
  // and "bundle" for bundles - there's no separate "game" type. We exclude
  // bundles here (bundle appids aren't valid for appdetails lookups) but
  // otherwise keep results as-is; DLC/soundtracks are rare in a name search
...
// Returns true if the current discount/price satisfies the user's chosen filter. No filter set (both null) = "any discount at all" - the bot's
// doriginal default behavior.
function meetsCondition(thresholdPercent, thresholdPriceCents, actualDiscountPercent, actualPriceCents) {
  if (thresholdPercent == null && thresholdPriceCents == null) {
    return actualDiscountPercent > 0;
  }
  const percentMet = thresholdPercent != null && actualDiscountPercent >= thresholdPercent;
  const priceMet = thresholdPriceCents != null && actualPriceCents != null && actualPriceCents <= thresholdPriceCents;
  return percentMet || priceMet;
}
